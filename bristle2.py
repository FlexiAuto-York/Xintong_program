...
class Bristle(object):
    ...
    def step(self, action):
        ...
        if (self.goal_name == 'Goal2'):
            # print(self.lastdis,dis)

            err_code, self.target_position_handle = sim.simxGetObjectHandle(self.clientID, self.goal_name,
                                                                            sim.simx_opmode_blocking)
            ret, self.target_position = sim.simxGetObjectPosition(self.clientID, self.target_position_handle, -1,
                                                                  sim.simx_opmode_blocking)
            dis = cau_dis2(self.position, self.target_position)
            costs = (math.pow((20 - dis), 2) - math.pow((20 - self.lastdis), 2))
            # (self.lastdis - dis) * 100
